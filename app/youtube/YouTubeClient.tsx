...
export default function YouTubeClient({ stats, totals, params, sort }: any) {
    ...
    const handleManualGenerate = async () => {
        if (!baseUrl) { alert("Please enter your Typeform URL first."); return; }
        const vidId = extractVideoId(manualVideoUrl);
        const link = `${baseUrl}${baseUrl.includes('?') ? '&' : '?'}utm_source=youtube&utm_medium=organic${vidId ? `&utm_content=${vidId}` : ''}`;
        navigator.clipboard.writeText(link);
        setManualCopied(true);
        setTimeout(() => setManualCopied(false), 2000);

        if (vidId && !stats.some((v: any) => v.id === vidId) && !optimisticVideos.some((v: any) => v.id === vidId)) {
            const newVideo = { id: vidId, title: "New Tracked Video", thumbnail: `https://i.ytimg.com/vi/${vidId}/mqdefault.jpg`, leads: 0, qualified: 0, cash: 0, revenue: 0, calls: 0, taken: 0, closed: 0, savedUrl: baseUrl };
    ...
    const handleCardCopy = (video: any) => {
        ...
        if (!targetUrl) { alert("Enter URL above first."); return; }
        navigator.clipboard.writeText(`${targetUrl}${targetUrl.includes('?') ? '&' : '?'}utm_source=youtube&utm_medium=organic&utm_content=${video.id}`);
            ...
            <div className="max-w-[1600px] mx-auto space-y-12">
                ...
                <div className="bg-zinc-900/40 border border-zinc-800/80 backdrop-blur-sm rounded-2xl p-8 mb-8 shadow-lg relative z-0">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                        <input type="text" placeholder="Landing Page URL" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} className="bg-[#0a0a0a] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none" />
                        <input type="text" placeholder="YouTube URL" value={manualVideoUrl} onChange={(e) => setManualVideoUrl(e.target.value)} className="bg-[#0a0a0a] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none" />
                    </div>
                    <button onClick={handleManualGenerate} className="w-full bg-white hover:bg-zinc-200 text-black font-black uppercase tracking-widest text-xs py-4 rounded-xl transition-colors">
                        {manualCopied ? "Link Copied!" : "Generate Tracking Link"}
                    </button>
                </div>
                ...
                <div className="space-y-6 relative z-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {activeVideos.map((video: any) => (
                            <VideoCard key={video.id} video={video} onCopy={handleCardCopy} onArchive={() => toggleArchive(video.id)} isCopied={copiedId === video.id} />
                        ))}
                    </div>
                </div>
                ...
                {archivedVideos.length > 0 && (
                    <div className="mt-20 border-t border-zinc-800 pt-10">
                        <button onClick={() => setShowArchive(!showArchive)} className="flex items-center gap-3 mx-auto mb-10 px-6 py-3 bg-zinc-900/50 border border-zinc-800 rounded-full hover:bg-zinc-800 transition-colors">
                        ...
                        {showArchive && (
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 opacity-60">
                                {archivedVideos.map((video: any) => (
                                    <VideoCard key={video.id} video={video} onCopy={handleCardCopy} onArchive={() => toggleArchive(video.id)} isCopied={copiedId === video.id} isArchived />
                                ))}
                            </div>
                        )}
                    </div>
