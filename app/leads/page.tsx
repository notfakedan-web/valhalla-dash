...
import { Users, Filter, TrendingUp, Search, Globe, DollarSign, Loader2 } from 'lucide-react';
  ...
  try {
    ...
    return rows.map(row => {
        const get = (search: string[]) => {
            const key = sheet.headerValues.find(h => search.some(s => h.toLowerCase().includes(s.toLowerCase())));
...
async function LeadsContent({ params }: any) {
    ...
    const uniquePlatforms = Array.from(new Set(allLeads.map((l: any) => l.source))).filter(Boolean);
    ...
    const recentLeads = filteredLeads.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 20);

    // 3. CASH ON HAND BREAKDOWN
    ...
    if (!graphStart && filteredLeads.length > 0) { 
        const times = filteredLeads.map((d: any) => new Date(d.date).getTime()).filter(t => !isNaN(t)); 
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <div className="space-y-6 relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <HeroCard label="Total Leads" value={totalLeads.toLocaleString()} icon={<Users size={18} className="text-blue-400" />} accentColor="blue" />
                        <HeroCard label="Qualified (High Ticket)" value={qualifiedLeads.toLocaleString()} icon={<Filter size={18} className="text-indigo-400" />} accentColor="indigo" />
                        <HeroCard label="Qualification Rate" value={`${qualificationRate.toFixed(1)}%`} icon={<TrendingUp size={18} className="text-emerald-400" />} accentColor="emerald" />
                    </div>
                                ...
                                <div className="absolute inset-0 z-10 pl-2 pb-6 flex items-end justify-between">
                                    {trend.map(([date, count], i) => (
                                        <div key={i} className="flex-1 h-full flex flex-col justify-end items-center group relative cursor-crosshair hover:bg-white/5 transition-colors rounded-lg">
                                            <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity duration-200 bottom-0 mb-2 pointer-events-none transform translate-y-[-10px] group-hover:translate-y-0"
                                    ...
                                    return (
                                        <div key={i} className="group">
                                            <div className="flex justify-between items-end mb-2">
                                                <span className="text-[11px] font-bold text-zinc-300 uppercase tracking-wide group-hover:text-white transition-colors truncate max-w-[150px]">{range}</span>
                                            ...
                                            <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                                                <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full transition-all duration-1000 ease-out" style={{ width: `${pct}%` }} />
                                            </div>
                                        </div>
                                    )
                                })}
...
function getFundsStyle(funds: string) {
    ...
    if (funds.match(/10k|5k|20k/i)) return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
    if (funds.match(/3k/i)) return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    ...
    return (
        <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center space-y-1">
                <h3 className="text-lg font-bold text-white uppercase tracking-widest animate-pulse">Loading Leads...</h3>
            </div>
        </div>
    );
}
