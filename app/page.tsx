...
export default async function ValhallaDashboard({ searchParams }: { searchParams: Promise<any> }) {
  ...
  // 1. ACCOUNTING FILTER (By Timestamp) - This gets you the $15,486 Cash
  ...
  // 2. PERFORMANCE FILTER (By Date Call Was Taken) - This gets you the 82% Show Rate
  ...
  const totalRev = accountingData.reduce((acc, curr) => acc + curr.revenue, 0);

  // Filter out MRR, Downsells, and Tests for Sales Metrics
  ...
  const callsClosed = appointments.filter(d => ['closed', 'deposit collected', 'paid'].some(x => d.outcome.toLowerCase().includes(x))).length;
  ...
  // TREND CHART (Daily Cash based on Timestamp)
    ...
    <div className="min-h-screen bg-black text-white p-10 font-sans">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-4">
           <div className="lg:col-span-2 bg-cyan-600 rounded-xl p-8">
              <p className="text-[9px] font-black text-white/50 uppercase mb-2">Cash Collected</p>
           ...
           <div className="lg:col-span-2 bg-zinc-900/30 rounded-xl p-8 border border-zinc-800">
              <p className="text-[9px] font-black text-zinc-600 uppercase mb-2">Revenue Generated</p>
            ...
            {trend.map(([date, cash], i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                <div className="relative w-full">
                  ...
                  <div className="w-full bg-cyan-500 rounded-sm hover:bg-cyan-400 transition-all" style={{ height: `${Math.max(4, (cash / (totalCash||1)) * 1200)}px` }} />
                </div>
