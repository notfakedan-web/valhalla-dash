...
async function getLeadsData() {
  try {
    ...
    return rows.map(row => {
      ...
      // Normalize Date: Handle DD/MM/YYYY or MM/DD/YYYY variations
      const rawDate = getVal('DATE');
      let parsedDate = new Date(rawDate);
      
      // If JS fails to parse, try manual split (common for spreadsheet dates)
      if (isNaN(parsedDate.getTime()) && rawDate.includes('/')) {
        const parts = rawDate.split('/');
        // Assumes MM/DD/YYYY - change order if your sheet is DD/MM
...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<any> }) {
  ...
  const filtered = allLeads.filter(d => {
    if (!d.isoDate) return true; // Keep if date is unparseable to avoid losing data
    if (params.start && d.isoDate < new Date(params.start)) return false;
    if (params.end && d.isoDate > new Date(params.end)) return false;
  ...
  return (
    <div className="min-h-screen bg-black text-white p-10 font-sans">
      <div className="max-w-[1600px] mx-auto">
                  ...
                  <svg className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none">
                      <polyline
                          ...
                          points={timeTrend.map(([_, count], i) => {
                              ...
                              const svgY = 300 - (count / maxDayLeads) * 250;
                              return `${svgX},${svgY}`;
                          }).join(' ')}
                ...
                <div className="absolute inset-0 flex">
                    {timeTrend.map(([date, count], i) => (
                        <div key={i} className="flex-1 flex flex-col justify-end group items-center relative">
                            <div 
                                className="w-2.5 h-2.5 bg-cyan-400 rounded-full z-10 border-2 border-black transition-all group-hover:scale-150" 
        ...
        <div className="bg-gradient-to-r from-cyan-600 to-blue-600 rounded-2xl p-8 mb-8 text-center">
            <p className="text-[9px] font-black text-white/50 uppercase mb-2 tracking-widest">Total Leads</p>
            <h2 className="text-5xl font-black">{filtered.length}</h2>
        </div>

        {/* CASH CHART */}
        <div className="bg-[#0d0d0d] border border-zinc-900 rounded-3xl p-10 mb-8">
            <h3 className="text-[10px] font-black uppercase text-zinc-600 tracking-widest mb-12 text-center">Cash On Hand</h3>
            <div className="h-[250px] flex items-end justify-around gap-4 px-10">
                {cashBrackets.map(([bracket, count], i) => (
                    <div key={i} className="flex-1 flex flex-col items-center group">
                        <div 
                            className="w-full max-w-[50px] bg-cyan-500/20 border-t-2 border-cyan-400 transition-all group-hover:bg-cyan-500/40" 
                            style={{ height: `${(count / (filtered.length || 1)) * 200}px` }} 
                        />
                        <span className="text-[8px] font-black text-zinc-500 mt-4 uppercase text-center truncate w-full">{bracket}</span>
                ...
                <tbody className="divide-y divide-zinc-800/50">
                    {filtered.slice(0, 50).map((lead, i) => (
                        <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                            ...
                            <td className="p-6 text-cyan-400 font-black">{lead.cashOnHand}</td>
